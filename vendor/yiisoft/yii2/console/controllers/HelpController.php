...
class HelpController extends Controller
{
    ...
    protected function getDefaultHelp()
    {
        ...
        if (empty($commands)) {
            $this->stdout("\n".Yii::t('app','No commands are found.')."\n\n", Console::BOLD);
            return;
        }

        $this->stdout("\n".Yii::t('app','The following commands are available:')."\n\n", Console::BOLD);
        $maxLength = 0;
        ...
        foreach ($commands as $command => $description) {
            ...
            $this->stdout(Console::wrapText(Yii::t("app",$description), $maxLength + 4 + 2), Console::BOLD);
            $this->stdout("\n");
            $prefix = $controller->getUniqueId();
            foreach ($actions as $action) {
                $string = '  ' . $prefix . '/' . $action;
                ...
                if ($summary !== '') {
                    ...
                    $this->stdout(Console::wrapText(Yii::t('app',$summary), $maxLength + 4 + 2));
                }
                $this->stdout("\n");
            }
            $this->stdout("\n");
        ...
        $scriptName = $this->getScriptName();
        $this->stdout("\n".Yii::t('app',"To see the help of each command, enter:")."\n", Console::BOLD);
